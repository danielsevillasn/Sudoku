...
/**
 * Objeto que inicializa el tablero, la puntuacion, los errores, el tiempo y la
 * dificultad
 * 
 * @author Dani S
 */
public class SudokuModel implements Serializable {
    ...
    // Comprueba si un número se ha colocado correctamente las 9 veces
    // reglamentarias
    ...
    public boolean proponerValor(int fila, int columna, int valor) {
        if (celdasIniciales[fila][columna])
            return false;
    ...
    public boolean solicitarPista(int fila, int columna) {
        if (fila == -1 || columna == -1 || celdasIniciales[fila][columna])
            return false;
        if (tableroActual[fila][columna] == tableroSolucion[fila][columna])
            return false;
    ...
    public boolean[][] getCeldasIniciales() {
